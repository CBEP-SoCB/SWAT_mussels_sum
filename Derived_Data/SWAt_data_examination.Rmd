...
```{r}
tmp  %>% group_by(SAMPLE_ID, SAMPLE_DATE, `ANALYSIS LAB`, ANALYSIS_LAB_SAMPLE_ID, prefix, suffix) %>%
  summarize(n = n(),
            d = max(CONCENTRATION)- min(CONCENTRATION),
            .groups = 'drop') %>%
  filter(n>1)
```
Those appear to be  duplicates with exactly the same value in CONENTRATION, and no other obvious discrepancies.
...
```
So, looking at that site we again see no differences. This appears to be a sample duplication in the data record.  It's not the only one.

Since these are apparent duplicates, we lose no information by  averaging the two values.  Here we check to make sure the different totals fall in the expoected sequence order.
```{r}
tmp %>%
