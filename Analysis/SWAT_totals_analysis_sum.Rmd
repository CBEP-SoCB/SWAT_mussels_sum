...
focus here on various sums of organic contaminants that can be related
more readily to reference levels.
...
edge. That positioning could be relegated to a parameter.
...
Give the paucity of data and sites from 2015 through 2019, we consider "recent"
...
PAHs are also highly correlated.

### PAH 40
So, we focus on PAH 40, since it matches a published benchmark, and approximates
total PAHs fairly well.  Our primary interest in the status analysis is to
compare observations to benchmarks. We do this principally graphically.
...
#### Log Linear Model
Analternative approach is to explicitly model this as a log linear model and
extract expectations and confidence intervals from the model object, using
`emmeans`. This makes the results model dependent, but makes more efficient use
of the data (if the model is appropriate).
...
models show marked scale-dependence of variability, which is largely missing here.
We'd be far better off with a model that better fit the random component of
these data.
...
misleading. In particular, the error bars for the smaller means are effectively
"infected" with the high variability of the larger means. Despite no variability
to speak of at Navy Pier, we get significant standard errors because variability
is estimated based on all sites.
...
That is actually fairly remarkable, and suggests an ANCOVA would be informative.
...
*  The Mill Creek (CBMCMC) Samples are a factor of just under three (exp(1)) 
   smaller that the other two sites.
...
pcb_data %>%
  ...
  cor(use='pairwise')
```
So even though SWAT PCBs are based on a subset of all PCBs, the two metrics are 
highly correlated. The TEQs are only moderately correlated with the other two 
measures.
...
concentrations at spring point appear to have been declining.  However, given
variability in observations, that could be due to chance.
